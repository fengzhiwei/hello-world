...
namespace PipeLine.Class
{
    class ChartToExcel
    {
        ...
        private void LoadData(System.Data.DataTable dataTable)
        {
            ...
            {
                for (int j = 0; j < dt_test.Columns.Count -1; j++)
                {
                    string kk = dt_test.Rows[i - 1][j].ToString();
                    xlSheet.Cells[i, j + 1] = dt_test.Rows[i - 1][j].ToString();
                }
            }
        }
        private void CreateChart()
        {
            ...
            Microsoft.Office.Interop.Excel.Series s2 = (Microsoft.Office.Interop.Excel.Series)grp.SeriesCollection(2);
            s2.BarShape = XlBarShape.xlCylinder;
